...
export default function LoadsClient({ loads }: Props) {
  ...
  return (
    <div>
      <section>
        {loads.length === 0 ? (
          <div className="bg-surface bg-opacity-5 p-12 text-center rounded-none">
          ...
          <div className="overflow-x-auto border border-gray-700 rounded-none">
            <table className="w-full bg-surface bg-opacity-5">
              <thead>
              ...
              <tbody>
                {loads.map((load, index) => (
                  ...
                  >
                    <td className="px-6 py-imperial text-gray-200 font-medium">{load.origin_zip}</td>
                    <td className="px-6 py-imperial text-gray-200 font-medium">{load.destination_zip}</td>
                    <td className="px-6 py-imperial text-gray-200">{load.load_weight.toLocaleString()} lbs</td>
                    <td className="px-6 py-imperial text-gray-200">{load.commodity_type}</td>
                    <td className="px-6 py-imperial text-gray-400 text-sm">{new Date(load.created_at).toLocaleDateString()}</td>
                    <td className="px-6 py-imperial">
                      <button
                        onClick={() => openModal(load.id)}
                        className="inline-flex items-center justify-center px-4 py-2 bg-forest text-white hover:bg-forest/80 focus:ring-2 focus:ring-charcoal-black transition font-semibold text-sm rounded-none"
      ...
      {showToast && toastMessage && (
        <div className="fixed top-8 left-1/2 transform -translate-x-1/2 z-[100]">
          <div className="bg-forest px-12 py-6 ">
