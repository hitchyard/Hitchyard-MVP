...
export default function CarrierDashboardClient({ 
  ...
  const [performanceScore, setPerformanceScore] = useState<number | null>(null);
    // Fetch carrier performance metrics (Key Performance Metrics)
    useEffect(() => {
      async function fetchPerformance() {
        try {
          ...
          if (json.success && json.data) {
            // All metrics normalized to 1-10 for HPS formula
            ...
            // Calculate Hitchyard Performance Score (HPS)
            ...
            setPerformanceScore(Number(score.toFixed(2)));
          }
        } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-charcoal-black">
      ...
      {/* HITCHYARD PERFORMANCE SCORE DASHBOARD */}
      <section className="py-[100px] border-b border-white/5">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h3 className="text-2xl font-serif font-bold uppercase tracking-[0.6em] text-white mb-10">Hitchyard Performance Score (HPS)</h3>
          <div className="flex flex-col items-center justify-center mb-8">
            <div className="bg-green-900/80 rounded-full w-40 h-40 flex flex-col items-center justify-center border-4 border-green-400 shadow-lg">
              <span className="text-6xl font-serif font-bold text-white">{performanceScore !== null ? performanceScore : '--'}</span>
              <span className="text-xs font-sans text-white/70 uppercase tracking-[0.4em] mt-2">/ 10</span>
            </div>
            <p className="text-white font-sans text-sm mt-4">Composite score based on reliability, efficiency, relationship yield, and AI match success. Key performance metrics for trustworthy logistics.</p>
          </div>
